...
export async function browseDirectoryQuickPick(directoryPath: string, history: string[]) {
  try {
    ...
    quickPick.onDidTriggerButton(async (button) => {
      if (button === vscode.QuickInputButtons.Back) {
        ...
        await browseDirectoryQuickPick(previousDirectoryPath, history)
      }
    })
    ...
    quickPick.onDidChangeSelection(async ([selectedItem]) => {
      if (selectedItem) {
        if (selectedItem.isDirectory) {
          await browseDirectoryQuickPick(selectedItem.uri.fsPath, [...history, directoryPath])
        } else {
